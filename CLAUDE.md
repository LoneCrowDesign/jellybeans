...
- **Examples and docs:** each bean ships a buildable `examples/` sketch that compiles the library in place, and a README covering its API and wire contract.
...
A schema-driven async web console. The architectural idea is that the device is self-describing. It advertises a manifest of its commands, variables, files, uploads, and record collections over one WebSocket, and a single embedded browser page builds its entire UI from that manifest. Adding a feature is "register a command or a variable"; the transport and the UI never change. Nothing about the page is per-project.
...
- **Registration:** happens before `begin()`, which loads persisted variables and publishes the manifest.
- **`Field` arrays:** stored by pointer, and must have static storage duration. This applies to both command argument schemas and collection field schemas, which share the same machinery.
- **Name and help strings:** stored by pointer too, not copied.
...
- **NVS keys:** ≤ 15 chars, so persisted variable names must stay short. Persistence uses `Preferences` (NVS); only the log file uses `cfg.fs`.
- **Filesystem:** WebConsole never mounts one. `cfg.fs` is whatever the host already mounted, and null is valid (RAM log tail only).
- **Command arg schemas:** exist for type safety, not decoration. Without one, `led on=true` could only reach the wire as the string `"true"`, and a handler reading `a["on"] | false` would silently see a non-bool and do the opposite of what was typed. That is why untyped `key=val` input is rejected.
