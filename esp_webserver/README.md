...
single embedded browser page builds itself from that manifest — log pane, a
typed command console, a type-aware variable table.
...
example. On 8/16 MB parts this is rounding error;
RAM is the real constraint — each WebSocket client holds heap for TCP +
framing, so cap concurrent clients on no-PSRAM boards.
...
| `io()` | A `Print&` sink — use it like `Serial`; each `\n`-terminated line becomes a log entry. |
...
| `upload_result` | `{name, ok, result}` — after an upload is validated in `loop()`. |
| `collection_changed` | `{name, ok, result}` — after a create/edit/delete; clients re-fetch that collection. |
...
| `POST /api/file/clear` | `name` (+ `token`) — erase a clearable registered file. |
...
press Enter. Commands are not enumerated as buttons by default — that keeps
...
> `addVar` escape hatch, keep the getter cheap and pure — no hardware access
...
- **Device-initiated events:** the `event` message type is reserved and rendered
  ...
  broadcasts.
- **Editing the UI:** `WebConsoleUI.h` is a plain editable raw-string page — no
  build/minify step. Keep it self-contained (no external assets) so it works
  offline on a SoftAP.
...
- **NVS keys are ≤ 15 chars.** Keep persisted variable names short.
- **`name` and `help` strings are stored by pointer, not copied.** Pass string
...
- **`<deviceName>.local` needs an mDNS client.** macOS/iOS/Windows 10+/Linux-with-avahi
  resolve it; Android does not resolve `.local` in the browser — those clients
  must connect by IP.
...
PSRAM, RAM is the binding constraint — cap concurrent WebSocket clients.
